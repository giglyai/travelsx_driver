...
class UserTripScreen extends StatefulWidget {
  ...
  UserTripScreen({Key? key, this.rideStatus, this.fromHome = false})
    : super(key: key);
...
class _UserTripScreenState extends State<UserTripScreen>
  ...
  void initState() {
    ...
    if (widget.fromHome == true) {
      tripEquatableCubit.changeOrderFilter(widget.rideStatus ?? TripStatus.all);
    } else {
      tripEquatableCubit.changeOrderFilter(TripStatus.all);
    }

    tripEquatableCubit.getSingleTripData(
      status: widget.rideStatus ?? TripStatus.all,
    );
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(
        bottom: widget.fromHome == false ? kBottomNavigationBarHeight : 0,
      ),
      child: Scaffold(
